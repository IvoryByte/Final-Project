...
Popular_movies %>%
  ...
  type_convert() %>%
  ggplot(aes(x = Year)) +
  geom_point(aes(y = Score), alpha = .3) +
  geom_line(aes(y = AverageScore), color = "maroon") 
#trying to get graph to work
...
Popular_movies %>%
  ...
  ggplot(aes(x = Year)) +
  geom_line(aes(y = AverageScore))
#Got rid of 2018 due to incomplete sample. There have been only 92 films so far. This means that it includes bad and good films, not just good ones.
```
